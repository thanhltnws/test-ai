...
### International market — data thiếu chiều sâu

`prompt.py` có fallback: nếu LLM không xác định được market từ record, mặc định gán `"international"`. Hệ quả: market `international` **tồn tại trong signals** nhưng đến từ fallback, không phải từ client được thiết kế cố ý cho thị trường đó.

Raw data hiện tại không có client international nào được xây dựng rõ ràng (ví dụ: Singapore, Australia, US). Số lượng và chất lượng signal `international` phụ thuộc vào tỉ lệ record mà LLM không infer được market — không kiểm soát được, không có giá trị demo ổn định.

**Cần làm:** thêm ít nhất 2–3 fictional international clients vào raw data (vd: Singapore fintech, US SaaS) để market `international` có signal có chủ đích, đủ để aggregate và hiển thị trên dashboard.
